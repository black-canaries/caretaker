/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsNumber, ValidateNested, IsOptional } from "class-validator";
import { UserCreateNestedManyWithoutInsulinsInput } from "./UserCreateNestedManyWithoutInsulinsInput";
import { Type } from "class-transformer";

@InputType()
class InsulinCreateInput {
  @ApiProperty({
    required: true,
    type: Number,
  })
  @IsNumber()
  @Field(() => Number)
  dose!: number;

  @ApiProperty({
    required: false,
    type: () => UserCreateNestedManyWithoutInsulinsInput,
  })
  @ValidateNested()
  @Type(() => UserCreateNestedManyWithoutInsulinsInput)
  @IsOptional()
  @Field(() => UserCreateNestedManyWithoutInsulinsInput, {
    nullable: true,
  })
  users?: UserCreateNestedManyWithoutInsulinsInput;
}

export { InsulinCreateInput as InsulinCreateInput };
